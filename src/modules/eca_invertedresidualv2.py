...
class ECAInvertedResidualv2(nn.Module):
    ...
    def __init__(self, inp, oup, stride, expand_ratio, k, norm_layer=None):
...
class ECAInvertedResidualv2Generator(GeneratorAbstract):
    ...
    def __call__(self, repeat: int = 1):
        ...
        """
        module = []
        _, t, s, k = self.args  # c is equivalent as self.out_channel
        inp, oup = self.in_channel, self.out_channel
        for i in range(repeat):
            stride = s if i == 0 else 1
            module.append(
                self.base_module(inp=inp, oup=oup, expand_ratio=t, stride=stride, k=k)
            )
            inp = oup
